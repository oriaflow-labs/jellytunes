...
function ok(stdout = 'secret-value'): SecretToolResult {
  return { status: 0, stdout };
}
function failed(status: number, stderr = 'boom'): SecretToolResult {
  return { status, stderr };
}

/** Capture every stdin chunk the runner receives. */
function captureStdin(run: SecretToolRunner): {
  runner: SecretToolRunner;
  writes: string[];
} {
  const writes: string[] = [];
  const wrapped: SecretToolRunner = (args, stdin) => {
    return run(args, (chunk: string) => writes.push(chunk));
  };
  return { runner: wrapped, writes };
}

describe('createSecretStore', () => {
  describe('store', () => {
    it('passes service=jellytunes to secret-tool store', async () => {
      const runner = vi.fn<SecretToolRunner>().mockReturnValue(ok());
      const store = createSecretStore({ runner });

      await store.store('my-secret');

      expect(runner).toHaveBeenCalledWith(
        ['store', '--label=jellytunes-session', 'service', 'jellytunes'],
        expect.any(Function),
      );
    });
    ...
    it('writes the full secret to stdin and closes the stream', async () => {
      const { runner, writes } = captureStdin(
        vi.fn<SecretToolRunner>().mockReturnValue(ok()) as unknown as SecretToolRunner,
      );
      const store = createSecretStore({ runner });
    ...
    it('throws on non-zero exit code', async () => {
      const runner = vi.fn<SecretToolRunner>().mockReturnValue(failed(1, 'keyring locked'));
      const store = createSecretStore({ runner });
  ...
  describe('lookup', () => {
    it('calls secret-tool lookup with the matching service attribute', async () => {
      const runner = vi.fn<SecretToolRunner>().mockReturnValue(ok('recovered'));
      const store = createSecretStore({ runner });

      const result = await store.lookup();

      expect(runner).toHaveBeenCalledWith(['lookup', 'service', 'jellytunes'], expect.any(Function));
    ...
    it('returns null when the entry does not exist (exit code 1)', async () => {
      ...
      const runner = vi.fn<SecretToolRunner>().mockReturnValue(failed(1, ''));
      const store = createSecretStore({ runner });
    ...
    it('throws on unexpected exit codes with stderr attached', async () => {
      const runner = vi.fn<SecretToolRunner>().mockReturnValue(failed(2, 'portal died'));
      const store = createSecretStore({ runner });
    ...
    it('trims trailing newline that secret-tool appends', async () => {
      const runner = vi.fn<SecretToolRunner>().mockReturnValue(ok('value\n'));
      const store = createSecretStore({ runner });
  ...
  describe('clear', () => {
    it('calls secret-tool clear with the service attribute', async () => {
      ...
      expect(runner).toHaveBeenCalledWith(['clear', 'service', 'jellytunes'], expect.any(Function));
    });
  ...
  describe('availability probe', () => {
    it('reports available when secret-tool runs and answers a lookup', async () => {
      const runner = vi.fn<SecretToolRunner>().mockReturnValue(ok('ok'));
      const store = createSecretStore({ runner });
    ...
    it('reports unavailable when secret-tool exits 127 (not installed)', async () => {
      const runner = vi.fn<SecretToolRunner>().mockReturnValue(failed(127, 'command not found'));
      const store = createSecretStore({ runner });

      await expect(store.isAvailable()).resolves.toBe(false);
    });

    it('reports unavailable on ENOENT (binary missing)', async () => {
      const runner = vi.fn<SecretToolRunner>().mockImplementation(() => {
  ...
  describe('timeout handling', () => {
    it('the runner contract treats a null status as timeout (not a hang)', async () => {
      ...
      // hang indefinitely.
      const runner: SecretToolRunner = () => ({ status: null, stderr: 'timed out' });
    ...
    it('store rejects with a timeout-shaped error on status:null', async () => {
      const runner: SecretToolRunner = () => ({ status: null, stderr: 'killed' });
      const store = createSecretStore({ runner, timeoutMs: 50 });
