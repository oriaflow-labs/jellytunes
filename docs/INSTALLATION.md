...
> **Known issue — session persistence under Snap on some distributions.** Inside the Snap, libsecret routes session storage through the desktop's Secret portal (`org.freedesktop.portal.Secret`) — it never talks to the Secret Service D-Bus API directly, which is why no `password-manager-service` connection is needed (connecting it has no effect; verified). Whether sessions persist therefore depends on the host desktop providing a working Secret portal backend. Confirmed working on Ubuntu 26.04. Confirmed broken on Zorin OS 18: the portal request fails, the app shows "sessions will not persist", and you are asked to log in again on every launch. This is host-side portal configuration, not confinement — no AppArmor denials are involved, and forcing a backend via `~/.config/xdg-desktop-portal/portals.conf` did not resolve it on Zorin 18. On affected systems, install the `.deb` instead — it runs unconfined and is unaffected. Diagnostics land in the app log (`~/snap/jellytunes/current/.config/JellyTunes/logs/`) as `secret-tool` records carrying an `operation` and a `stderrClassification`.
